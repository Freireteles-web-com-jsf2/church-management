...
async function testSecurityAuditSystem() {
  ...
  const authSuccessEvent = await SecurityAuditService.logAuthenticationEvent(
    'LOGIN_SUCCESS',
    'SUCCESS',
    'admin@igreja.com',
    'user-123',
    ...
    { loginMethod: 'local', sessionDuration: 3600 }
  );
  ...
  const authFailureEvent = await SecurityAuditService.logAuthenticationEvent(
    'LOGIN_FAILURE',
    'FAILURE',
    ...
    '10.0.0.1',
    'Malicious Bot',
    { failureReason: 'INVALID_PASSWORD', attemptCount: 5 }
  );
  ...
  const userCreateEvent = await SecurityAuditService.logUserManagementEvent(
    'USER_CREATED',
    'SUCCESS',
    'admin-123',
    'admin@igreja.com',
  ...
  const passwordChangeEvent = await SecurityAuditService.logPasswordEvent(
    'PASSWORD_CHANGED',
    'SUCCESS',
    'user-123',
    'user@igreja.com',
    '192.168.1.100',
    { changeReason: 'USER_INITIATED', strengthScore: 85 }
  );
  ...
  const sessionEvent = await SecurityAuditService.logSessionEvent(
    'SESSION_CREATED',
    'SUCCESS',
    'user-123',
    'user@igreja.com',
    'session-token-789',
    '192.168.1.100',
    { rememberMe: true, expiresIn: 2592000 }
  );
  ...
  const violationEvent = await SecurityAuditService.logSecurityViolation(
    'BRUTE_FORCE_ATTEMPT',
    'attacker-999',
    'attacker@malicious.com',
    '10.0.0.1',
    { attemptCount: 15, timeWindow: 300, targetAccounts: ['admin@igreja.com', 'user@igreja.com'] }
  );
  ...
  // Test basic query
  const allEvents = await SecurityAuditService.queryAuditEvents({ limit: 10 });
  console.log(`   Total de eventos encontrados: ${allEvents.length}`);

  // Test filtered queries
  const authEvents = await SecurityAuditService.queryAuditEvents({
    eventTypes: ['AUTHENTICATION'],
    limit: 10
  });
  console.log(`   Eventos de autenticação: ${authEvents.length}`);

  const highRiskEvents = await SecurityAuditService.queryAuditEvents({
    riskScoreMin: 50,
    limit: 10
  });
  console.log(`   Eventos de alto risco (≥50): ${highRiskEvents.length}`);

  const failureEvents = await SecurityAuditService.queryAuditEvents({
    outcomes: ['FAILURE'],
    limit: 10
  });
  ...
  // Test search
  const searchResults = await SecurityAuditService.queryAuditEvents({
    searchText: 'LOGIN',
    limit: 10
  });
  ...
  for (let i = 0; i < 8; i++) {
    await SecurityAuditService.logAuthenticationEvent(
      ...
      '192.168.1.200',
      'Suspicious Browser',
      { failureReason: 'INVALID_PASSWORD', attemptNumber: i + 1 }
    );
  }
  ...
  const loginHistory = await SecurityAuditService.getDetailedLoginHistory('user-123', 10);
  ...
  const highRiskEvent = await SecurityAuditService.logAuditEvent({
  ...
  const customEvent = await SecurityAuditService.logAuditEvent({
  ...
  const finalSummary = await SecurityAuditService.getAuditSummary();
  ...
  const finalSuspiciousActivities = await SecurityAuditService.detectSuspiciousActivity();
